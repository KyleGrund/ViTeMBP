...
/**
 * Class providing an interface to the VL6180X sensor.
 */
class DistanceVL6180X extends DistanceSensor {
    /**
     * A UUID representing the type of this sensor.
     */
    static final UUID TYPE_UUID = UUID.fromString("416ffd9b-67ac-4cb3-9c04-30ba1f8640ca");
     ...
     */
    DistanceVL6180X(String name, String calData) {
        super(name);
        // decode cal data of the form "([min],[max])"
        if (calData != null && !calData.isEmpty()) {
            String[] split = calData.split(",");
            if (split.length != 2) {
                throw new IllegalArgumentException("VL6180X calibration data must be of the form \"([min],[max])\".");
            }
            ...
            try {
                ...
                throw new IllegalArgumentException("VL6180X calibration data must be of the form \"([min],[max])\".", ex);
            }
